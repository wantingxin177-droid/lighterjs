...
import type { FundingPoint } from "@/types/data";
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine, Legend } from "recharts";
import { Button } from "@/components/ui/button";

const COLORS = ["#34d399", "#60a5fa", "#f472b6", "#fbbf24", "#a78bfa", "#fb7185", "#22d3ee", "#f97316"];

export function FundingChart({ fundings }: { fundings: Record<string, FundingPoint[]> }) {
  const symbols = Object.keys(fundings);
  const [selected, setSelected] = useState<string[]>(symbols.slice(0, 4));
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {symbols.map((s, i) => (
          <Button
            ...
            onClick={() => toggle(s)}
            className={selected.includes(s) ? "bg-emerald-600 hover:bg-emerald-500 text-white" : "border-zinc-700 text-zinc-400"}
            style={selected.includes(s) ? { backgroundColor: COLORS[i % COLORS.length], borderColor: "transparent" } : {}}
          >
      ...
      <p className="text-xs text-zinc-500">7 天每小时资金费率（%）。正值 = 多头付空头。按钮旁标注了按 8 小时结算折算的年化费率。</p>
    </div>
  );
}
